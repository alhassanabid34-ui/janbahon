...
async function razorpay(env, path, options = {}) {
  const response = await fetch(`https://api.razorpay.com/v1${path}`, {
    ...options,
    headers: { authorization: basicAuth(env.RAZORPAY_KEY_ID, env.RAZORPAY_KEY_SECRET), ...(options.headers || {}) }
  });
...
async function loadTicket(db, bookingId) {
  const booking = await db.prepare(`SELECT b.booking_id,b.bus_id,b.journey_date,b.total_amount,b.payment_status,b.created_at,
    bu.operator,bu.name AS bus_name,bu.bus_type,bu.from_city,bu.to_city,bu.departure,bu.arrival,bu.duration,
    COALESCE(o.full_name,'') AS owner_name
    FROM bookings b JOIN buses bu ON bu.bus_id=b.bus_id LEFT JOIN owners o ON o.owner_id=bu.owner_id WHERE b.booking_id=?`).bind(bookingId).first();
...
export async function onRequestPost({ request, env }) {
  ...
  const orderIdFromClient = String(body?.razorpay_order_id || "");
  ...
  try {
    ...
    if (attempt.status === "paid") {
      ...
      return json({ success: true, alreadyProcessed: true, bookingId: attempt.booking_id, totalAmount: ticket?.total_amount || attempt.amount, whatsappSent: true });
    }

    const valid = await verifySignature(attempt.razorpay_order_id, paymentId, signature, env.RAZORPAY_KEY_SECRET);
    ...
    if (payment.status === "authorized") {
      payment = await razorpay(env, `/payments/${encodeURIComponent(paymentId)}/capture`, {
        ...
        body: JSON.stringify({ amount: Number(attempt.amount) * 100, currency: "INR" })
      });
    }
    ...
    if (!booking) return json({ error: "Booking reservation has expired or was already completed." }, 409);
    ...
    try {
      whatsapp = await sendBookingWhatsApp(env, ticket, pdf);
    } catch (error) {
      console.error("WhatsApp ticket delivery failed", error);
      whatsapp = { sent: false, skipped: false, reason: String(error?.message || error) };
    }
