(() => {
  ...
  function show(html){css(); if(modal) modal.remove(); modal=document.createElement('div'); modal.className='jbal-overlay'; modal.innerHTML='<div class="jbal-card"><button class="jbal-x">×</button>'+html+'</div>'; document.body.appendChild(modal); modal.querySelector('.jbal-x').onclick=close; return modal;}
  function close(){if(modal){modal.remove();modal=null;}}
  function error(e){const m=modal&&modal.querySelector('.jbal-msg'); if(m){m.textContent=e.message||String(e);m.className='jbal-msg';}}
  function login(){
    const m=show('<h2>Passenger Login</h2><p>Login using your mobile number and password.</p><form id="jbalForm"><label>Mobile number</label><input id="jbalMobile" inputmode="numeric" maxlength="10" placeholder="10-digit mobile number"><label>Password</label><input id="jbalPass" type="password" placeholder="Password"><div class="jbal-msg" style="display:none"></div><button class="jbal-btn">Login</button></form><button class="jbal-link" id="jbalOtp">Login with SMS OTP</button><button class="jbal-link" id="jbalSignup">Create account</button><button class="jbal-link" id="jbalReset">Forgot password?</button>');
  ...
  function otpVerify(mobile,action,title){
    const m=show('<h2>'+title+'</h2><p>Enter the 6-digit SMS OTP sent to +91 '+esc(mobile)+'.</p><form id="jbalForm"><label>SMS OTP</label><input id="jbalOtp" inputmode="numeric" maxlength="6" placeholder="6-digit OTP"><div class="jbal-msg" style="display:none"></div><button class="jbal-btn">Verify</button></form>');
  ...
  function signupOtp(mobile,name,password){
    const m=show('<h2>Verify Mobile</h2><p>Enter the OTP sent to +91 '+esc(mobile)+'.</p><form id="jbalForm"><label>SMS OTP</label><input id="jbalOtp" inputmode="numeric" maxlength="6" placeholder="6-digit OTP"><div class="jbal-msg" style="display:none"></div><button class="jbal-btn">Create Account</button></form>');
